...
void apply_context_defaults(Args& args) {
    if (args.context.conditionLabel.empty()) {
        args.context.conditionLabel = ipi::api::default_condition_label("udp", args.context.networkLoadLevel);
    }
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            std::cout
                ...
                << "  --host <ip>                  UDP receiver host (default 127.0.0.1)\n"
                << "  --port <port>                UDP receiver port (default 36667)\n"
...
ipi::MessageFrame build_service_frame(const Args& args, std::uint64_t sequence) {
    ...
    if (args.payloadBytes > 0) {
        ...
        message.offloadTaskId = "udp-probe-" + std::to_string(sequence);
    }
...
ipi::api::ExperimentLogRecord make_base_record(const Args& args,
    ...
    record.emitterRole = "5g-udp-sender";
...
void record_result(const Args& args,
                   const ipi::api::Private5gProbeRequest& request,
...
void record_failure(const Args& args,
                    const ipi::api::Private5gProbeRequest& request,
                    std::string detail,
                    std::uint64_t emitTimeNs,
                    ProbeStats& stats) {
...
void print_summary(const ProbeStats& stats, bool csv) {
    ...
    const auto successRate =
        total == 0 ? 0.0 : (100.0 * static_cast<double>(stats.accepted) / static_cast<double>(total));
