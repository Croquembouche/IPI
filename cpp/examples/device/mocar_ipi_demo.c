...
#define SPAT_MAX_PHASES 8
#define DEFAULT_SPAT_PHASES 2
#define SPAT_OVERRIDE_PATH_ENV "MOCAR_SPAT_FILE"
...
struct spat_override_entry {
    int signal_group;
    int phase_state;
    int likely_time_ms;
};

struct spat_override {
    int count;
    struct spat_override_entry entries[SPAT_MAX_PHASES];
};

static int load_spat_override(struct spat_override* out)
{
    const char* path = getenv(SPAT_OVERRIDE_PATH_ENV);
    if (!path || !*path) {
        return 0; // no override configured
    }
    FILE* f = fopen(path, "r");
    if (!f) {
        perror("[SPaT] fopen override");
        return -1;
    }

    char line[128];
    int count = 0;
    while (fgets(line, sizeof(line), f) && count < SPAT_MAX_PHASES) {
        // skip comments/blank lines
        if (line[0] == '#' || line[0] == '\n') continue;
        struct spat_override_entry e;
        if (sscanf(line, "%d %d %d", &e.signal_group, &e.phase_state, &e.likely_time_ms) == 3) {
            out->entries[count++] = e;
        }
    }
    fclose(f);
    out->count = count;
    if (count > 0) {
        fprintf(stdout, "[SPaT] loaded %d override phase(s) from %s\n", count, path);
    }
    return 0;
}
...
static int fill_spat(v2x_msg_spat_t* spat)
{
    ...
    struct spat_override ov = {0};
    if (load_spat_override(&ov) != 0) {
        // failed to load; keep defaults
        ov.count = 0;
    }
    int phase_count = (ov.count > 0) ? ov.count : DEFAULT_SPAT_PHASES;
    intersection->num_movement_states = phase_count;
    ...
    for (i = 0; i < phase_count; ++i) {
        ...
        if (ov.count > 0) {
            state->signal_group_id = ov.entries[i].signal_group;
            state->movement_event = calloc(1, sizeof(mde_MovementEventList_t));
            if (!state->movement_event) return -1;
            state->num_movement_event = 1;
            state->movement_event[0].phase_state = ov.entries[i].phase_state;
            state->movement_event[0].time_change_info.likely_time = ov.entries[i].likely_time_ms;
        } else {
