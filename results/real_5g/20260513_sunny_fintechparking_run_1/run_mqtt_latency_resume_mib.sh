...
PAPER_RUN_ID="edge4av-real-20260513-sunny-fintechparking-run-1-mqtt-latency"
REPO="/home/hydrau/Documents/Github/IPI"
RUN_DIR="$REPO/results/real_5g/$RUN_ID"
KEY="/tmp/ipi-ssh-$RUN_ID-mqtt-latency/key"
...
COUNT=1000
INTERVAL_MS=200
PAYLOADS=(1048576 2097152)
COMMON_ARGS=(
  --run-id "$PAPER_RUN_ID"
  --condition-label private-5g-sunny-fintechparking-mqtt-latency
  --av-id av-1
  --obu-id obu-1
  --rsu-id rsu-1
...
: > "$RUN_DIR/commands/mqtt_latency_resume_mib_commands.txt"
...
ssh_launch_detached() {
  ...
  timeout 5s ssh -i "$KEY" -o BatchMode=yes "$REMOTE" "$cmd" \
    >> "$RUN_DIR/mqtt_latency_resume_remote_launch_detach.log" 2>&1 || true
}
...
start_mqtt_broker() {
  ...
  cleanup_remote
  local cmd="cd '$REMOTE_RUN' && nohup python3 threaded_mqtt_broker.py --host 0.0.0.0 --port $MQTT_PORT > mqtt_latency_threaded_broker.log 2> mqtt_latency_threaded_broker.err < /dev/null & echo \$! > mqtt_latency_threaded_broker.pid"
  log_cmd "remote: $cmd"
  ...
  ssh_remote "cat '$REMOTE_RUN/mqtt_latency_threaded_broker.err' 2>/dev/null || true"
  return 1
}
...
run_mqtt_payload() {
  local payload="$1"
  local condition="p5g-mqtt-latency-payload-$payload"
...
{
  echo "# MQTT Latency MiB Resume Metadata"
  echo
  ...
  echo "- Broker: threaded Python MQTT 3.1.1 QoS0 broker, used after single-thread broker stalled on first 1 MiB attempt."
...
} > "$RUN_DIR/mqtt_latency_resume_mib_metadata.md"

echo "$(date --iso-8601=seconds) starting threaded MQTT broker"
start_mqtt_broker
