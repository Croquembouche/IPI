...
COUNT=1000
INTERVAL_MS=200
PAYLOADS=(0 256 1024 4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152)
COMMON_ARGS=(
  --run-id "$PAPER_RUN_ID"
...
: > "$RUN_DIR/commands/mqtt_latency_commands.txt"
...
ssh_launch_detached() {
  ...
  timeout 5s ssh -i "$KEY" -o BatchMode=yes "$REMOTE" "$cmd" \
    >> "$RUN_DIR/mqtt_latency_remote_launch_detach.log" 2>&1 || true
}

cleanup_remote() {
  ssh_remote "pkill -f '[e]xample_private_5g_latency_receiver.*--transport mqtt' || true; pkill -f '[m]inimal_mqtt_broker.py --host 0.0.0.0 --port $MQTT_PORT' || true" || true
}
...
start_mqtt_broker() {
  ...
  local cmd="cd '$REMOTE_RUN' && nohup python3 minimal_mqtt_broker.py --host 0.0.0.0 --port $MQTT_PORT > mqtt_latency_broker.log 2> mqtt_latency_broker.err < /dev/null & echo \$! > mqtt_latency_broker.pid"
  log_cmd "remote: $cmd"
  ...
  ssh_remote "cat '$REMOTE_RUN/mqtt_latency_broker.err' 2>/dev/null || true"
  return 1
}
...
{
  echo "# MQTT Latency Sweep Metadata"
  echo
...
} > "$RUN_DIR/mqtt_latency_metadata.md"
