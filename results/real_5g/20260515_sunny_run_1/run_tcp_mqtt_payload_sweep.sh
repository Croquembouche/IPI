...
: > "$RUN_DIR/commands/payload_sweep_commands.txt"
...
start_mqtt_broker() {
  ...
  ssh_remote "pkill -f '[t]hreaded_mqtt_broker.py --host 0.0.0.0 --port $MQTT_PORT' || true; pkill -f '[m]inimal_mqtt_broker.py --host 0.0.0.0 --port $MQTT_PORT' || true"
...
{
  echo "# TCP/MQTT Payload Sweep Metadata"
  echo
...
} > "$RUN_DIR/payload_sweep_metadata.md"
