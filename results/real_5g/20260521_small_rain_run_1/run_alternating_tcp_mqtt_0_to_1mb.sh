...
RUN_ID="20260521_small_rain_run_1"
PAPER_RUN_ID="edge4av-real-20260521-small-rain-run-1"
...
COMMON_ARGS=(
  --run-id "$PAPER_RUN_ID"
  --condition-label private-5g-small-rain
  --av-id av-1
  --obu-id obu-1
  --rsu-id rsu-1
...
start_mqtt_receiver() {
  ...
  local cmd="cd '$REMOTE_REPO' && mkdir -p '$REMOTE_RUN' && nohup stdbuf -oL -eL ./cpp/build/example_private_5g_latency_receiver --transport mqtt --host 127.0.0.1 --port $MQTT_PORT --intersection-id small-rain-run-1 --source-id av-1 --run-id '$PAPER_RUN_ID' --condition-id '$condition' --condition-label private-5g-small-rain --rsu-id rsu-1 --network-load-level idle --qos-profile default --mobility-state stationary --clock-sync-state unsynced --service-success true --csv > '$out' 2> '$err' < /dev/null & echo \$! > '$pid'"
  log_cmd "remote: $cmd"
  ssh_launch_detached "$cmd"
  sleep 2
...
run_mqtt_payload() {
  ...
  local cmd=(./cpp/build/example_private_5g_latency_sender
    ...
    --message service --payload-bytes "$payload"
    --intersection-id small-rain-run-1 --source-id av-1
    --condition-id "$condition" --request-id "$condition"
    "${COMMON_ARGS[@]}"
    --csv)
  log_cmd "local: ${cmd[*]} > $RUN_DIR/${condition}_sender.csv 2> $RUN_DIR/${condition}_sender.err"
