...
: > "$RUN_DIR/commands/full_payload_sweep_commands.txt"
...
ssh_launch_detached() {
  ...
  timeout 5s ssh -i "$KEY" -o BatchMode=yes "$REMOTE" "$cmd" \
    >> "$RUN_DIR/full_sweep_remote_launch_detach.log" 2>&1 || true
}
...
{
  echo "# Full TCP/MQTT Payload Sweep Metadata"
  echo
  ...
  echo "- Weather: sunny after rain"
  echo "- Vehicle location: moved to new location before run"
  echo "- Mobility: stationary"
  echo "- Clock sync state: unsynced"
  echo "- MQTT broker: threaded Python MQTT 3.1.1 QoS0 broker on base station"
  echo "- Timestamp note: sender send time is patched after request encoding and immediately before TCP send or MQTT publish, so sender-side packaging time is excluded from RTT."
} > "$RUN_DIR/full_payload_sweep_metadata.md"
